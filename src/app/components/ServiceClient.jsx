...
export default function ServiceClient({ data }) {
  ...
  return (
    <div ref={container} className="bg-[#F3F3F3] text-black min-h-screen selection:bg-black selection:text-white">
      
      {/* FLOATING CONTACT (Minimal) */}
      <div className="fixed bottom-8 right-8 z-50 mix-blend-difference text-white">
        <Link href="/contact" className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest hover:underline decoration-1 underline-offset-4">
                    ...
                    <div className="grid grid-cols-2 gap-8">
                        {data.stats.map((stat, i) => (
                            <div key={i} className="reveal-text">
                                <span className="block text-4xl font-light">{stat.value}</span>
            ...
            <div className="md:col-span-8 md:pl-12 border-l border-black/5">
                ...
                {data.longDescription && (
                    <div className="text-neutral-600 text-lg leading-relaxed max-w-2xl mb-16 reveal-text space-y-6">
                        ...
                        <p>We approach every {data.title.toLowerCase()} project with a focus on longevity, sustainability, and aesthetic precision.</p>
                    </div>
                )}

                {/* Minimal Accordion / List for Features */}
                <div className="space-y-0">
                    <h3 className="text-xs font-mono uppercase text-neutral-400 mb-6 tracking-widest">Key Features</h3>
                    {data.features.map((feature, i) => (
                        <div key={i} className="group border-t border-black/10 py-8 transition-all hover:bg-white hover:px-6 -mx-6 px-6 cursor-default">
                            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                                <div className="flex items-center gap-4">
                                    <span className="text-xs font-mono text-neutral-300">0{i+1}</span>
                                    <h4 className="text-xl font-medium group-hover:translate-x-2 transition-transform">{feature.title}</h4>
                                </div>
      ...
      <section className="bg-white py-24 px-6 md:px-12 border-t border-black/5">
         <div className="max-w-[1800px] mx-auto">
            <div className="flex items-end justify-between mb-16">
                <h2 className="text-4xl md:text-6xl font-light tracking-tight reveal-text">The Process</h2>
                <ArrowDownRight size={40} strokeWidth={1} className="text-neutral-300" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-neutral-200 border border-neutral-200">
                {data.process.map((step, i) => (
                    <div key={i} className="bg-white p-10 h-full flex flex-col justify-between hover:bg-neutral-50 transition-colors">
                        <div>
                            <span className="block text-xs font-mono text-neutral-400 mb-6">STEP {step.step}</span>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-20">
             ...
             <div className="flex flex-col justify-center items-start md:pl-20">
                 <h2 className="text-5xl md:text-7xl font-light tracking-tighter mb-8 leading-[0.9]">
                     Ready to <br/><span className="italic font-serif">begin?</span>
                 </h2>
                 <Link href="/contact" className="group inline-flex items-center gap-4 text-xl border-b border-black pb-1 hover:pb-2 transition-all">
